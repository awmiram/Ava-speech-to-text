...
export default function AudioPlayer({ src , tab }) {
  ...
  return (
    <div className="bg-gray-50 mb-4 px-5 rounded-xl border-gray-100 flex items-center gap-4" dir="ltr">
      {src && (
        <video  ref={audioRef}  src={src}  onTimeUpdate={handleTimeUpdate}  onLoadedMetadata={handleLoadedMetadata}  onEnded={() => setIsPlaying(false)}  onWaiting={() => setIsLoading(true)}  onPlaying={() => setIsLoading(false)}  className="hidden"/>
      )}
      ...
      <button onClick={togglePlay} disabled={!src} className="w-8 h-8 rounded-full flex items-center justify-center transition-colors cursor-pointer disabled:bg-gray-300 disabled:cursor-not-allowed shrink-0">
        {isLoading ? (
          <svg className="size-5 animate-spin text-gray-500" fill="none" viewBox="0 0 24 24">
